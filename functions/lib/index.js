...
function resolveEarningAmount(booking) {
...
function isVolunteerCredit(booking) {
...
function resolveSettlementRoute(booking) {
    ...
    for (const txDoc of eligibleSnapshot.docs) {
        ...
        await db.runTransaction(async (trx) => {
            ...
            const basePending = walletSnap.exists
                ? walletSnap.get('pendingBalance') ?? 0
                : 0;
            const baseAvailable = walletSnap.exists
                ? walletSnap.get('availableBalance') ?? 0
                : 0;
            const baseVolunteer = walletSnap.exists
                ? walletSnap.get('volunteerBalance') ?? 0
                : 0;
            const currency = walletSnap.exists
                ? walletSnap.get('currency') ?? 'USD'
                : 'USD';
            const kycStatus = walletSnap.exists
                ? walletSnap.get('kycStatus') ?? 'none'
                : 'none';
            const withdrawalThreshold = walletSnap.exists
                ? walletSnap.get('withdrawalThreshold') ?? 500
                : 500;
...
export const bookingStatusPolicyHook = onDocumentUpdated('bookings/{bookingId}', async (event) => {
    ...
    if (typeof after.teacherId !== 'string'
    ...
    if (after.status === 'accepted') {
        ...
        await db.runTransaction(async (trx) => {
            ...
            const learnerAvailable = learnerWalletSnap.exists
                ? learnerWalletSnap.get('availableBalance') ?? 0
                : 0;
            const learnerPending = learnerWalletSnap.exists
                ? learnerWalletSnap.get('pendingBalance') ?? 0
                : 0;
            const learnerVolunteer = learnerWalletSnap.exists
                ? learnerWalletSnap.get('volunteerBalance') ?? 0
                : 0;
            const learnerCurrency = learnerWalletSnap.exists
                ? learnerWalletSnap.get('currency') ?? 'USD'
                : 'USD';
            const learnerKyc = learnerWalletSnap.exists
                ? learnerWalletSnap.get('kycStatus') ?? 'none'
                : 'none';
            const learnerThreshold = learnerWalletSnap.exists
                ? learnerWalletSnap.get('withdrawalThreshold') ?? 500
                : 500;
            if (learnerAvailable < amount) {
                trx.set(settlementRef, {
                    bookingId,
                    learnerId,
                    teacherId,
                    amount,
            ...
            const routeEscrowBalance = routeFundSnap.exists
                ? routeFundSnap.get('escrowBalance') ?? 0
                : 0;
            trx.set(learnerWalletRef, {
                uid: learnerId,
    ...
    const settlementRef = db.doc(`booking_settlements/${bookingId}`);
    const now = Timestamp.now();
    const availableAt = Timestamp.fromMillis(now.toMillis() + (7 * 24 * 60 * 60 * 1000));
    let payoutCreated = false;
    let payoutSkippedReason = 'unknown';
    let payoutRouteUsed = settlementRoute;
    await db.runTransaction(async (trx) => {
        ...
        const basePending = teacherWalletSnap.exists
            ? teacherWalletSnap.get('pendingBalance') ?? 0
            : 0;
        const baseAvailable = teacherWalletSnap.exists
            ? teacherWalletSnap.get('availableBalance') ?? 0
            : 0;
        const baseVolunteer = teacherWalletSnap.exists
            ? teacherWalletSnap.get('volunteerBalance') ?? 0
            : 0;
        const currency = teacherWalletSnap.exists
            ? teacherWalletSnap.get('currency') ?? 'USD'
            : 'USD';
        const kycStatus = teacherWalletSnap.exists
            ? teacherWalletSnap.get('kycStatus') ?? 'none'
            : 'none';
        const withdrawalThreshold = teacherWalletSnap.exists
            ? teacherWalletSnap.get('withdrawalThreshold') ?? 500
            : 500;
        ...
        releaseAmount = settlementSnap.get('amount') ?? amount;
        const settlementStoredRoute = settlementSnap.get('route') ?? null;
        const effectiveRoute = settlementStoredRoute === 'teacher_direct'
            ? 'teacher_direct'
            : 'organization_escrow';
        ...
        const settlementStatus = settlementSnap.get('status') ?? 'held';
...
export const walletWithdrawalRequestHook = onDocumentCreated('wallet_withdrawal_requests/{requestId}', async (event) => {
    ...
    await db.runTransaction(async (trx) => {
        ...
        const availableBalance = walletSnap.get('availableBalance') ?? 0;
        const pendingBalance = walletSnap.get('pendingBalance') ?? 0;
        const volunteerBalance = walletSnap.get('volunteerBalance') ?? 0;
        const currency = walletSnap.get('currency') ?? 'USD';
        const kycStatus = walletSnap.get('kycStatus') ?? 'none';
        const withdrawalThreshold = walletSnap.get('withdrawalThreshold') ?? 500;
...
export const reviewQualityPolicyHook = onDocumentCreated('reviews/{reviewId}', async (event) => {
    ...
    const teacherId = typeof review.teacherId === 'string' && review.teacherId.length > 0
        ? review.teacherId
        : null;
    if (teacherId == null) {
    ...
    await db.runTransaction(async (trx) => {
        ...
        const teachingEnabled = userSnap.exists
            ? userSnap.get('teachingModeEnabled') ?? false
            : false;
        const recentReviewsSnap = await trx.get(db
            .collection('reviews')
        ...
        let nextTeachingModeEnabled = null;
